...
extension Contador_de_horas_laboralControl {
    struct Value {
        var isRunning: Bool
        var name: String
    }
    ...
    struct Provider: AppIntentControlValueProvider {
        func previewValue(configuration: TimerConfiguration) -> Value {
            Contador_de_horas_laboralControl.Value(isRunning: false, name: configuration.timerName)
        }

        func currentValue(configuration: TimerConfiguration) async throws -> Value {
            let isRunning = true // Check if the timer is running
            return Contador_de_horas_laboralControl.Value(isRunning: isRunning, name: configuration.timerName)
        }
    }
}
